<template>
  <div>
    ...
    <el-dialog title="添加用户" :visible.sync="addDialogVisible" width="50%" @close="addDialogClosed">
      <el-form :model="form" :rules="addFormRules" ref="addFormRef" label-width="70px">
        ...
        <el-form-item label="状态" prop="status">
          ...
          <el-radio v-model="form.status" label="hidden">隐藏</el-radio>
        </el-form-item>
      </el-form>
    ...
    <el-dialog title="修改用户" :visible.sync="editDialogVisible" width="50%" @close="editDialogClosed">
      <el-form :model="form" :rules="editFormRules" ref="editFormRef" label-width="70px">
        <el-form-item label="用户名" prop="username">
          <el-input v-model="form.username"></el-input>
        </el-form-item>
        <el-form-item label="密码" prop="password">
    ...
    methods: {
      ...
      addSave() {
        this.$refs.addFormRef.validate(async valid => {
          if (!valid) return
          const { data: res } = await this.$http.post('auth/admin/add', this.form)
          if (res.status !== 200) {
            return this.$message({
              type: 'warning',
              message: res.msg
      ...
      editSave() {
        this.$refs.editFormRef.validate(async valid => {
          if (!valid) return
          const { data: res } = await this.$http.put('auth/admin/' + this.form.id, this.form)
          if (res.status !== 200) {
            return this.$message({
              type: 'warning',
              message: '修改失败'
