...
import { getAdminList, adminStateChange, addSave } from '../../api/auth'
...
export default {
  ...
  methods: {
    ...
    async adminStateChange(admininfo) {
      const { msg: message } = await adminStateChange(
    ...
    addSave() {
      this.$refs.addFormRef.validate(async (valid) => {
        if (!valid) return
        const { msg: message } = await addSave(this.addForm)
        this.$message({
          type: 'success',
          message: message
        })
    ...
    editSave() {
      this.$refs.editFormRef.validate(async (valid) => {
        if (!valid) return
        console.log(this.this.editForm.id)
        const { msg: message } = await function (url) {
          return http({
            url: 'auth/admin/' + this.editForm.id,
            data: this.editForm
          })
        }
        this.$message({
