...
export default {
  ...
  watch: {
    tags: function (newQuestion, oldQuestion) {
      window.localStorage.setItem('tags', JSON.stringify(newQuestion))
    }
  },
  methods: {
    logout() {
    ...
    async getMenuList() {
      const { data: res } = await getMenus('menus')
      if (res.status !== 200) {
        return this.$message({
          message: res.msg,
          type: 'warning'
    ...
    closeRouter(index) {
      if (this.$route.path === this.tags[index].router) {
        if (index !== 0) {
          ...
          this._tabSessionStorage(this.tags[index - 1].router)
        } else if (this.tags.length > 1) {
          this.$router.push(this.tags[1].router)
          this._tabSessionStorage(this.tags[1].router)
        } else {
          this.$router.push('/welcome')
          this._tabSessionStorage('/welcome')
        }
      }
      this.tags.splice(index, 1)
    },
    _tabSessionStorage(router) {
      window.sessionStorage.setItem('activePath', router)
      this.activePath = router
    }
  }
...
.nav-tab li:hover .el-icon-close {
